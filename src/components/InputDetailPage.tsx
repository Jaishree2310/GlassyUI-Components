...
const Input: React.FC<InputProps> = ({
  ...
  return (
    <div className="relative">
      ...
      {icon && (
        <span
          className={`absolute top-1/2 transform -translate-y-1/2 ${iconPosition === 'left' ? 'left-3' : 'right-3'
            } cursor-pointer`}
          onClick={onIconClick}
        >
          {icon}
...
const InputDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    updateCustomCode();
  }, [customBgColor, customTextColor, customBorderColor, customOpacity, customBorderWidth, customBorderRadius]);
  ...
  style={{
    backgroundColor: '${customBgColor}${Math.round(customOpacity * 2.55).toString(16).padStart(2, '0')}',
    color: '${customTextColor}',
    borderColor: '${customBorderColor}',
    borderWidth: '${customBorderWidth}px',
    borderRadius: '${customBorderRadius}px',
